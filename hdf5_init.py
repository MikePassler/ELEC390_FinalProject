...
df = pd.read_csv('C:/Users/micha/ELEC390_FINALPROJECT/test_train.csv')
#Matt_data = pd.read_csv('C:/Users/micha/ELEC390_FINALPROJECT/test_train.csv')
...
grouped_data = data.groupby('window')['Acceleration z (m/s^2)'].agg(['mean', 'std', 'max', 'min', 'median', 'var', lambda x: skew(x)]).rename(columns={'<lambda_0>': 'skewness'})
...
# reorder the columns
grouped_data = grouped_data[['windows', 'mean', 'std', 'max', 'min', 'median', 'var', 'range', 'skewness']]

# display the result
print(grouped_data)

grouped_data["mean"] = grouped_data["mean"].apply(lambda x: 1 if x >= 2.6 else 0)
labels = grouped_data['mean']
DATA = grouped_data.drop('mean', axis = 1)
DATA = DATA.iloc[:, 1:]
print(DATA)
print(labels)
